...
if [ "$1" == "-h" ]; then
  ...
  echo ""
  echo "Takes a stranded SJ.out.tab file of introns generated by STAR aligner, with the number of junction reads in column7, and outputs a bed track like the one output by TopHat for viewing in IGV"
  exit 0
fi
...
echo "Converting..."
awk \
{'if($4=="2") print ""$1"\t"$2-$9-1"\t"$3+$9"\tJUNC_"$1"_"$2"_"$3"_-\t"$7"\t-\t"$2-$9-1"\t"$3+$9"\t255,0,0\t2\t"$9","$9"\t","0,"$3-$2+$9+1; \
else \
if($4=="1") print ""$1"\t"$2-$9-1"\t"$3+$9"\tJUNC_"$1"_"$2"_"$3"_+\t"$7"\t+\t"$2-$9-1"\t"$3+$9"\t0,0,255\t2\t"$9","$9"\t","0,"$3-$2+$9+1'} \
